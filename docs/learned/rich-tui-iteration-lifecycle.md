...
- BEADS snapshot loading uses `bd --readonly list --json --all --limit 0 --no-pager` (with fallback to non-readonly list when needed) so TUI state tracks live DB-backed bead status.
