...
## Design and behavior
- Keep lifecycle telemetry (`START`, `RUN`, `BATCH`, `TOKENS`, `PAUSE`, `RETRY`, picked tasks, stop marker) in the same live state model as the existing panel renderers.
...
- Rich-mode task snapshots are refreshed per iteration from TSQ output so panel state tracks current task status.
...
- Task snapshot loading uses `tsq list --json` with a timeout and shows an unavailable state when the command fails.

## Pitfalls and mitigations
- `OpenTuiLiveRunRenderer` can be unavailable at runtime; fallback renderer uses the same state contract so lifecycle behavior still stays replacement-first when not TTY.
...
- Snapshot loading remains bounded and command-based to avoid tight polling loops.
